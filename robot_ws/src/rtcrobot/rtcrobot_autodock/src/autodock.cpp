  ...
  charging_timeout_set_(false)
{
  ...
  //dock_pub_ = n.advertise<nav_msgs::Odometry>("dock", 50);
...
void AutoDocking::dockCallback(const rtcrobot_actions::DockGoalConstPtr& goal)
{
  ...
  debug_pose_.publish(dock_pose_base_link);
  ...
  else if (ros::ok() && continueDocking(result))
  {
    ...
    while (
           !controller_.approach(0.55, dock_pose_base_link) && 
           //continueDocking(result)             &&
           ros::ok()
           )
    {
      //ROS_INFO("OK3");
      perception_.getPose(dock_pose_base_link, "base_footprint");
      double dock_yaw2 = angles::normalize_angle(tf::getYaw(dock_pose_base_link.pose.orientation));
      //ROS_INFO("Yaw: %f, X: %f, Y: %f", dock_yaw2, dock_pose_base_link.pose.position.x, dock_pose_base_link.pose.position.y);
      //debug_pose_.publish(dock_pose_base_link);
      geometry_msgs::TransformStamped odom_trans;

      odom_trans.header.stamp = ros::Time::now();
      odom_trans.header.frame_id = "base_footprint";
      odom_trans.child_frame_id = "dock_link";

      odom_trans.transform.translation.x = dock_pose_base_link.pose.position.x;
      odom_trans.transform.translation.y = dock_pose_base_link.pose.position.y;
      odom_trans.transform.translation.z = 0.0;
      odom_trans.transform.rotation = dock_pose_base_link.pose.orientation;
