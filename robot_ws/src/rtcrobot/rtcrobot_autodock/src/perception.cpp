  ...
  found_dock_(false)
{
  ...
  float a_arr[] = {1.50000, -0.36953, 0.19582};
  ...
  // Create ideal cloud
  // Front face is 350mm long
  for (double y = 0.0; y <= 0.10; y += 0.001)
  {
  ...
  for (double x = 0; x < 0.10 /*0.0707106*/; x += 0.001)
  {
    geometry_msgs::Point p;
    p.x = x;
    p.y = 0.10 + x;
  ...
  // Each side is 100mm long, at 45 degree angle
  for (double x = 0.1; x > 0 /*0.0707106*/; x -= 0.001)
  {
    geometry_msgs::Point p;
    p.x = x;
    p.y = 0.30 - x;
  ...
  // Each side  is 300mm long
  for (double y = 0.30; y <= 0.35; y += 0.001)
  {
    geometry_msgs::Point p;
    p.x = p.z = 0.0;
    p.y = y;
  ...
  scan_sub_ = nh.subscribe("f_scan", 1, &DockPerception::callback, this);
  //cloud_sub_ = nh.subscribe("cloud_1", 1, &DockPerception::cloudcallback, this);
...
bool DockPerception::getPose(geometry_msgs::PoseStamped& pose, std::string frame)
{
  ...
  if (ros::Time::now() > dock_stamp_ + ros::Duration(10.0))
  {
...
void DockPerception::cloudcallback(const sensor_msgs::PointCloud2& cloud)
{
    ROS_INFO("Cloud recived data");
}
...
void DockPerception::callback(const sensor_msgs::LaserScanConstPtr& scan)
{
      ...
      (dock_.pose.orientation.z == 0.0 && dock_.pose.orientation.w == 0.0))
  {
    ...
    for (size_t i =  scan->ranges.size()*2/3; i < scan->ranges.size(); i++)
    {
      if (std::isfinite(scan->ranges[i]))
      {
        ...
        // ROS_INFO("dock_perception", "Set initial pose to (%f, %f, %f)",
        //                 dock_.pose.position.x, dock_.pose.position.y,
        //                 icp_2d::thetaFromQuaternion(dock_.pose.orientation));
        break;
      }
       
       size_t j = scan->ranges.size() - i;
      if (std::isfinite(scan->ranges[j]))
      {
        double angle = scan->angle_min + j * scan->angle_increment;
        dock_.header = scan->header;
        dock_.pose.position.x = cos(angle) * scan->ranges[j];
        dock_.pose.position.y = sin(angle) * scan->ranges[j];
        dock_.pose.orientation.x = 1.0;
        dock_.pose.orientation.y = 0.0;
        dock_.pose.orientation.z = 0.0;
        dock_.pose.orientation.w = 0.0;
  ...
  if (dock_.header.frame_id != tracking_frame_)
  {
    ...
    ROS_DEBUG_NAMED("dock_perception", "Transformed initial pose to (%f, %f, %f)",
                    dock_.pose.position.x, dock_.pose.position.y,
                    icp_2d::thetaFromQuaternion(dock_.pose.orientation));
    ROS_INFO("Transformed initial pose to (%f, %f, %f)",
                    dock_.pose.position.x, dock_.pose.position.y,
                    icp_2d::thetaFromQuaternion(dock_.pose.orientation));
  }
  ...
  if (!best)
  {
    ...
    ROS_WARN("DID NOT FIND THE DOCK");
    return;
  }
  ...
  else
  {
    ...
    if (d > 0.05)
    {
      ...
      //ROS_WARN("Dock pose jumped: %d", d);
      return;
    }
  }

  // Filter the pose esitmate.
  dock_.pose = best_pose;//dock_pose_filter_->filter(best_pose);
  dock_stamp_ = scan->header.stamp;
  found_dock_ = true;
  //ROS_INFO("x=%d, y=%d", dock_.pose.position.x, dock_.pose.position.y);
...
double DockPerception::fit(const DockCandidatePtr& candidate, geometry_msgs::Pose& pose)
{
  ...
  if (!isValid(init_pose))
  {
    ...
    ROS_INFO("Initial dock orientation estimate is invalid.");
    return -1.0;
  }
  ...
  if (!isValid(cand_pose))
  {
    ...
    ROS_INFO("Dock candidate orientation estimate is invalid."); 
  }
